...
function usage {
      cat <<EOF
   usage: $0 [options]
      trigger build of ${proj_PREFIX}soft continuous intergration build.
   options:
      --nevents     Define the number of events to process
      --step        Define the step number used to parse the right testmask column number
      ...
      --step-name   Define the name of the test
...
function initialize
{
    ...
    do
      ...
      x--step-name)  STEP_NAME="${2}";                                                              shift; shift;;
    ...
    check_data_production=$(sed -n '1p' ${TESTMASK} | cut -d ' ' -f ${STEP})
    check_compare_names=$(sed -n '2p' ${TESTMASK} | cut -d ' ' -f ${STEP})
    check_compare_size=$(sed -n '3p' ${TESTMASK} | cut -d ' ' -f ${STEP})
...
function novasoft_data_production
{
    TASKSTRING="novasoft_data_production"
    trap 'LASTERR=$?; echo -e "\nCI MSG BEGIN\n `basename $0`: error at line ${LINENO}\n Stage: ${STEP_NAME}\n Task: ${TASKSTRING}\n exit status: ${LASTERR}\nCI MSG END\n"; exit ${LASTERR}' ERR
    ...
    then
        ...
        echo -e "\nNumber of events for ${STEP_NAME} step: $NEVENTS\n"
        echo nova --rethrow-all -n ${NEVENTS} -o ${OUTPUT_STREAM} --config ${FHiCL_FILE} ${INPUT_FILE}
        echo

        nova --rethrow-all -n ${NEVENTS} -o ${OUTPUT_STREAM} --config ${FHiCL_FILE} ${INPUT_FILE}
    else
        echo -e "\nCI MSG BEGIN\n Stage: ${STEP_NAME}\n Task: ${TASKSTRING}\n skipped\nCI MSG END\n"
    fi
    exitstatus $?
...
function generate_data_dump
{
    ...
    echo "nova --rethrow-all -n ${NEVENTS} --config eventdump.fcl ${reference_file} > ${reference_file//.root}.dump"

    nova --rethrow-all -n ${NEVENTS} --config eventdump.fcl "${reference_file}" > "${reference_file//.root}".dump
    ...
    echo "nova --rethrow-all -n ${NEVENTS} --config eventdump.fcl ${current_file} > ${current_file//.root}.dump"

    nova --rethrow-all -n ${NEVENTS} --config eventdump.fcl "${current_file}" > "${current_file//.root}".dump
...
{       
    ...
    then 
        ...
        DIFF=$(diff  <(echo "${OUTPUT_REFERENCE//.}" | cut -d "|" -f -4 ) <(echo "${OUTPUT_CURRENT//.}" | cut -d "|" -f -4 ) )
        STATUS=$?
...
function compare_products_sizes
{
    ...
    then
        ...
        DIFF=$(diff  <(echo "${OUTPUT_REFERENCE//.}" ) <(echo "${OUTPUT_CURRENT//.}" ) )
        STATUS=$?
...
novasoft_data_production "${check_data_production}"
